...
by also using genre as a feature, attempted to use PCR and then followed the 
hint given in the book to explore the some of the more advanced models of the 
...
and for movie i and user u
$$ Y_{u,i} = \mu + b_i + b_u + \epsilon_{u,i} + \eta \sum_{g=1}^{k} b_g\theta_{g,i} $$
where
...
Plotting the bias of genres with 95% confidence intervals showed that the
bias based on the genre was not random. 
...
In the second part of the capstone, I looked beyond the simple bias models to
more sophisticated models, such as PCR. For much of this, I converted the test
and training datasets to matrices, and used the R  [recommnederlab](https://www.rdocumentation.org/packages/recommenderlab/versions/1.0.1) package
...
| Matrix | Dimension (rowsxcolumns)     | Sparcity (NA ratio) | 
|:--------|:---------------------------:|:-------------------:|
...
I used the following simple function to determine matrix sparcity
...
## Recommenderlab Setup and Workstation
...
Recommenderlab supports multiple different algorithms and is deigned to make them easy to compare.
In the capstone, I looked at 

* *RANDOM* (for a baseline)
* *UBCF*: User-based collborative filtering 
* *IBCF*: Item-based collborative filtering (IBCF)
* *SVD*: Singular value decomposition with column-mean imputation
* *SVDF*: Funk SVD

## RMSE for matrices with NAs

Recommenderlab has its own evaluation functions, but I extended the basic algorithm
to work with matrices with NAs.
...
## UBCF
UBCF looked to be very promising: tests against an initial small batch of users gave a substantially
...
and 90 minutes.
2. The recommendations produced had NA entries for some user/movie entries
